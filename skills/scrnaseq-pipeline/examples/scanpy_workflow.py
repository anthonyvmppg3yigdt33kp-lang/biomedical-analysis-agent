...
sc.pl.scatter(adata, x='total_counts', y='n_genes_by_counts', color='pct_counts_mt', ax=ax, show=False)
plt.tight_layout()
plt.savefig(f'{output_dir}/plots/qc_scatter.pdf')
plt.close()
...
# Expected rate must be justified from platform loading/recovery, not copied blindly.
expected_doublet_rate = min(0.15, max(0.005, 0.008 * adata.n_obs / 1000))
